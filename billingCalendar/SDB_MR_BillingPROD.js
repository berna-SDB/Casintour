...
define(['N/search', 'N/record', 'N/log'], function (search, record, log) {
    ...
    function getInputData() {
        const soSearch = search.create({
            ...
            filters: [
                ...
                'AND', ['custbody_sdb_order_processed_calendar', 'is', 'F'] //no fue procesada
            ],
    ...
    function reduce(context) {
        ...
        try {
            ...
            for (let i = 0; i < salesOrderIds.length; i++) { //Recorro todas las sales orders 
                ...
                for (let j = 0; j < lineCount; j++) { //recorro todas las lineas de la sale order
                    ...
                    if (itemType == 'Group') { //grupo de boletos 
                        ...
                        comissionSo.setCurrentSublistValue({ sublistId: 'item', fieldId: 'custcol_sdb_ticket_number', value: ticketNumber });
                        comissionSo.setCurrentSublistValue({ sublistId: 'item', fieldId: 'custcol_sdb_ticket_route', value: routes });
            ...
            log.debug('Sales Order principal creada', "https://11341630.app.netsuite.com/app/accounting/transactions/salesord.nl?id=" + newSoId + "&whence=");

            if (comissionSo.getLineCount({ sublistId: 'item' }) > 0) {
                var comissionSoId = comissionSo.save()
                log.debug('Sales Order por comisiones creada', "https://11341630.app.netsuite.com/app/accounting/transactions/salesord.nl?id=" + comissionSoId + "&whence=");
            }
            orderProcessed(salesOrderIds) //Marcar las ordenes como procesadas
        } catch (e) {
    ...
    // Marcar cada Sales Order original como procesada
    function orderProcessed(salesOrderIds) {
        salesOrderIds.forEach(function (soId) {
            try {
                record.submitFields({
                    type: record.Type.SALES_ORDER,
                    id: soId,
                    values: {
                        custbody_sdb_order_processed_calendar: true
                    }
                });
            } catch (e) {
                log.error('Error al marcar SO como procesada', e.message);
            }
        });
    }
