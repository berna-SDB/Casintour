...
define(['N/record', 'N/search', 'N/error'], function (record, search, error) {
    ...
    function createPurchaseBill(requestBody, ticketGroup, subsidiaryId) {
        ...
        tickets.forEach(function (ticket) {
            ...
            billRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'item', value: 273 });
    ...
    function createPurchaseIntercompanyBill(requestBody, customerSubsidiaryId, ticketGroup, subsidiaryId) {
        ...
        tickets.forEach(function (ticket) {
            ...
            billIntercompanyRecord.setCurrentSublistValue({ sublistId: 'item', fieldId: 'item', value: 273 });
    ...
    function createSalesOrder(requestBody, customer, ticketGroup) {
        ...
        tickets.forEach(function (ticket) { //por cada boleto creo un grupo con sus taxes
            ...
            salesOrder.setCurrentSublistValue({ sublistId: 'item', fieldId: 'item', value: 273 });
        ...
        if (customer.customerCalendary) {//si tiene calendario de facturacion entonces creamos la orden cerrada
            ...
            log.debug("#####Sales Order creada URL ", "https://11341630.app.netsuite.com/app/accounting/transactions/salesord.nl?id=" + salesOrderId + "&whence=");
        }
        else { //Entonces facturamos al momento 
            salesOrderId = salesOrder.save();
            log.debug("#####Factura creada URL ", "https://11341630.app.netsuite.com/app/accounting/transactions/salesord.nl?id=" + salesOrderId + "&whence=");
    ...
    function createIntercompanyInvoice(requestBody, subsidiaryId, customerSubsidiaryId, ticketGroup) {
        ...
        tickets.forEach(function (ticket) {
            ...
            intercompanyInvoice.setCurrentSublistValue({ sublistId: 'item', fieldId: 'item', value: 273 });
    ...
    function addCommission(requestBody, order, ticket, endLine) {
        ...
        if (ticket.comision > 0) {
            if (orig == "I") { //Si es internacional comision internacional 
                insertItem(order, line, 271, ticket.comision)
            }
            else if (orig == "D") { //si es local comision nacional 
                insertItem(order, line, 272, ticket.comision)
            }
        }
        else {
            return false;
    ...
    function setValuesToChargeItems(requestBody, order, ticket, startLine, endLine) {
        ...
        if (ticket.taxcombustible <= 0) {
            if (baseTaxEc == ticket.tarifa) {
                insertItem(order, lineNumber, 270, ticket.tarifa)
                lineNumber++
            }
            else {
                insertItem(order, lineNumber, 270, baseTaxEc)//Agregar Boleto con iva por la base del ec 
                lineNumber++;
                insertItem(order, lineNumber, 269, (ticket.tarifa - baseTaxEc)) //Agregar Boleto sin iva por la diferencia entre la Base del ec y la tarifa del boleto 
                lineNumber++
            }
        }
        else {
            if (baseTaxEc == (ticket.tarifa + ticket.taxcombustible)) {//Ambos tienen iva 
                insertItem(order, lineNumber, 270, ticket.tarifa)//Agregar Boleto con iva
                lineNumber++;
                insertItem(order, lineNumber, 264, ticket.taxcombustible)//Agregar combustible con iva 
                lineNumber++
            }
                ...
                if (baseTaxEc == ticket.tarifa) {
                    insertItem(order, lineNumber, 270, ticket.tarifa) // Agregar Boleto con iva
                    lineNumber++;
                    insertItem(order, lineNumber, 263, ticket.taxcombustible) //Agregar combustible sin iva 
                    lineNumber++
                }
                else if (baseTaxEc == ticket.taxcombustible) {
                    insertItem(order, lineNumber, 269, ticket.tarifa) //Agregar Boleto sin iva
                    lineNumber++;
                    insertItem(order, lineNumber, 264, ticket.taxcombustible) //Agregar combustible con iva 
                    lineNumber++
                }
        ...
        insertItem(order, lineNumber, 265, ticket.taxec)
        lineNumber++;
        insertItem(order, lineNumber, 266, ticket.taxed)
        lineNumber++;
        insertItem(order, lineNumber, 267, ticket.taxotro)
        lineNumber++
    ...
    function getVendor(vendorName) {
        ...
        else { return 2410 } //default vendor 
    }
    ...
    function logError(e, context = {}, history) {
        try {
            ...
            errorLog.setValue({ fieldId: 'custrecord_sdb_history', value: history });
